...
impl LlmState {
    ...
    pub async fn query(&self, word: &str) -> Result<String, String> {
        info!("Querying LLM for word: {}", word);
        ...
        debug!("LLM config loaded - api_base: {}, model: {}, timeout: {}s",
...
fn ensure_config(app: &tauri::AppHandle) -> Result<PathBuf, String> {
    ...
    debug!("LLM config directory: {:?}", config_dir);

    // 创建目录
    if !config_dir.exists() {
        info!("Creating LLM config directory");
        fs::create_dir_all(&config_dir)
            .map_err(|e| format!("Failed to create config directory: {}", e))?;
    }

    let config_path = config_dir.join("config.yaml");
    debug!("LLM config path: {:?}, exists: {}", config_path, config_path.exists());

    // 如果配置不存在，复制模板
    if !config_path.exists() {
        if let Some(template_path) = get_default_config_path(app) {
            info!("Copying LLM config template from {:?}", template_path);
            fs::copy(&template_path, &config_path)
                .map_err(|e| format!("Failed to copy config template: {}", e))?;
        } else {
            // 没有模板，创建默认配置
            info!("Creating default LLM config");
...
pub fn init_llm(app: &tauri::AppHandle) -> Result<(), String> {
    ...
    let config_path = if let Some(ref path) = dev_path {
        if path.exists() {
            debug!("Using dev LLM config: {:?}", path);
            path.clone()
        } else {
            // 生产模式：确保用户配置存在
            ensure_config(app)?
    ...
    info!("Using LLM config: {:?}", config_path);
